...
#This is a new csv file I created by renaming the header of the csv file using 
#python
#I found it way too difficult to rename 81 variable manually using rename() 
#[See --> change_to_r.py]
```
...
loan$listing.creation.date <- strptime(x = 
                                       as.character(loan$listing.creation.date), 
                                       format="%Y-%m-%d %H:%M:%S") 
#Convinient for extracting dates
...
# I found the dplyr package does not work properly with PosixXlt formats of 
#date-time. Therefore, I'll extract # dates wherever neccesary and convert this 
#column back to string after extraction.

loan$prosper.rating...alpha. <- factor(loan$prosper.rating...alpha.,
                                       c("AA","A","B","C","D","E","HR") , 
...
loan$income.range <- factor(loan$income.range , c('$0','$1-24,999','$25,000-49,999','$50,000-74,999','$75,000-99,999',
  '$100,000+','Not displayed','Not employed'), ordered =T) #ordering factors for ggplot

#Binning variable based on borrower APR.
...
loan.temp <- loan    
#Creating a temporary loan Structure so as to not modify the loaded csv data

#If prosper alpha is available , use that , else use the propser credit grade
#Alpha applicable for all loans >= 2008 , credit grade for all loans < 2008

loan.temp$prosper.rating...alpha. <- ifelse(is.na(loan.temp$prosper.rating...alpha.),
                                            as.character(loan.temp$credit.grade), as.character(loan.temp$prosper.rating...alpha.)) 

y <- (subset(loan.temp , !is.na(prosper.rating..numeric.)))

#nrow(y)

h <- hist(y$prosper.rating..numeric., breaks = c(0,1,2,3,4,5,6,7) , 
          ylim = c(0,20000) , xlab = 'Rating - Numeric' , col = 'red' ,
          main = 'Numeric Rating Histogram (Normal curve to scale)' ,
          labels = c('HR','E','D','C','B','A','AA')) 

xfit <- seq(min(y$prosper.rating..numeric.)-1 , max(y$prosper.rating..numeric.) ,
            length = nrow(y))

yfit <- dnorm(xfit, mean = mean(y$prosper.rating..numeric.),
              sd = sd(y$prosper.rating..numeric.))
...
loan$credit.score.exp <- 
  ((loan$credit.score.range.lower + loan$credit.score.range.upper)/2)

#New data again not to modify read csv data . But this contains average credit scores!
normalized.data <- subset(loan ,!is.na(credit.score.exp)) 

normalized.data$listing.creation.date <- 
  as.character(normalized.data$listing.creation.date)

#Converting to string because dplyr woudln't group a dataframe which has POSIXLT 
#format column
#I'll again convert it back to POSIXLT format when I need to extract the dates :)

#summary(loan$credit.score.exp) 

normalized.data.new <- subset(normalized.data , 
                  credit.score.exp > quantile(normalized.data$credit.score.exp, 0.05 ,
                                              na.rm = TRUE) &  credit.score.exp < quantile(normalized.data$credit.score.exp , 0.95 , na.rm = TRUE) &
                    prosper.rating...alpha. != 'AA') 
#Getting quantiles wise (reasons in the text ^)

#Seperate quantile for AA as mentioned above in the text ^^
normalized.data.aa <- subset(loan,prosper.rating...alpha. =='AA')

#Merging quantiles for all but AA loans , and AA loans (Hence complete!)
normalized.data <- rbind(normalized.data.aa, normalized.data.new) 

#Removing extremes - mainly because summary(normalized.data$credit.score.exp) 
#returned -9 as minimum which is entire possible for a borrower who is repeatedly
#delinquent - which doesnt help my plot!
...
normalized.data$credit.bins <- cut(normalized.data$credit.score.exp , 
                                   seq(609.5,889.5,15)) 
...
#Grouping data by credit bin --> Alpha score
...
#Subsetting only HR rated loans
normalized.data.groups.hr <- subset(normalized.data.groups , 
...
normalized.data.group.total <- subset(normalized.data.group.total ,
                                      median_limit < (789.5))

#Getting proportion by diving
normalized.data.groups.hr$n <-
  normalized.data.groups.hr$n / normalized.data.group.total$n


ggplot(data = subset(normalized.data.groups.hr , !is.na(credit.bins)) , 
       aes(x = credit.bins , y = n)) +
  geom_bar(stat = 'identity' ,aes(fill = I('aquamarine4'))) + 
  ggtitle('Ratio of HR loans with credit(bin wise)') +
  scale_y_continuous(breaks = seq(0,0.2,0.04) , labels = seq(0,0.2,0.04)) +
  ylab('Ratio')

rm(normalized.data.groups,normalized.data.group.total,
   normalized.data.groups.hr)

```

Next , I was analysing the distribution of Credit scores in the loan data. I decided to create a new variable , *credit.score.exp* which is the average of Uppper + lower bound credit scores!
Since credit score distribution is much like the distribution of alpha scores , I decided to look at non-performing loan distribution. I am plotting the distribution of Credit score , but only for **HR** rated loan.
...
ggplot(data = subset(loan,
   (!is.na(employment.status)) & employment.status != '' & employment.status != 'Not available' ),
...
suppressMessages(ggplot(data = loan , 
                        aes(x = employment.status.duration)) +
                   geom_freqpoly(aes(color = employment.status) , binwidth = 25) +
  scale_x_continuous(breaks = seq(0,550,25) , labels = seq(0,550,25)) +
   ylab('Frequency') +
...
suppressWarnings(ggplot(data = loan , aes(x = current.credit.lines)) + 
                   geom_freqpoly(aes(color = employment.status)) +
  scale_x_continuous(breaks = seq(0,40,2) , labels = seq(0,40,2)) +
 ggtitle('Plot of Credit Lines vs frequency') + 
  xlab('Current credit Lines') +
  scale_y_continuous(breaks = seq(0,20000,2000) , labels = seq(0,20000,2000))) +
  xlim(0,40) + scale_color_brewer(type = 'qual' , palette = 'Dark2') +
  theme_classic() 
...
#Function to transform loan category code --> loan category text
...
(Using Bi-variate plot here to explain the findings immediately above ^)
...
So far in Univariate plots , I've explored the distribution of various variables ,and how they fare.
...
* * *
I primarily use the **Observations** sub-category under Bi-variate plots because I think I can use these insights in bi-variate plots.
There were few plots which had really piqued my interest - namely plots {[Observation 1](#credit-score-distribution-by-alpha.) , [Plot 2](#ratio-of-hr-rated-loans.) , [Observation 3](#defaulting-rate-by-prosper-alpha-score.) , [Plot 11](#plot-11.) , [Plot 12](#plot-12.) , [Plot 14](#plot-14.) , [Observation 5](#borrowers-rate-for-alpha-score.) , [Observation 10](#observation-6.) }
...
cor.test(loan.2013$prosper.rating..numeric.,
         loan.2013$estimated.effective.yield) #Testing correlation
...
#Temporary debt dataframe to hold only between 5th and 95th qunatiles of data.
debt.temp <- subset(debt , 
                    loan.original.amount >quantile(debt$loan.original.amount , 0.05 ,
                                                   na.rm = TRUE) & 
                      loan.original.amount < quantile(debt$loan.original.amount , 0.95 ,
                                                      na.rm = TRUE))
...
bin <- function(x)  #Function to assign number to bin
{
  y = NA
  if(x == '(0,0.1]')
    y = 1
...
#I'm trying to test the correlation between debt to income ratio (binwise) and monthly loan payment , Althought I can simply test numeric vs numeric , I feel this way patterns are revelead , therefore I convert bin to a number here ^ and compare this number to monthly loan payment trends.

debt.temp$debt.value <- lapply(debt.temp$debt.bin,bin) #Applying function

debt.temp$debt.value <- as.numeric(debt.temp$debt.value) 
#Converting to numberic

debt.temp <- subset(debt.temp ,
                    monthly.loan.payment > quantile(debt.temp$monthly.loan.payment ,
                                                    0.05 , na.rm = TRUE) &
                      monthly.loan.payment < quantile(debt.temp$monthly.loan.payment ,
                                                      0.95 , na.rm = TRUE))

cor.test(debt.temp$debt.value , debt.temp$monthly.loan.payment ) #Correlation
...
normalized.data$listing.creation.date <- 
  as.character(normalized.data$listing.creation.date)
 #Converting to character --> DPLYR!

normalized.data$credit.bins <- cut(normalized.data$credit.score.exp , 
                                   seq(609.5,760,15)) 
#Binning variables by credit score
...
lmfit <- lm(borrower.rate ~ credit.bins , normalized.data) #Linear model
...
```{r , echo = FALSE , eval = FALSE}
cor.test(loan$open.credit.lines , loan$current.delinquencies) #Correlation

lmfit <- lm(current.delinquencies ~ open.credit.lines , loan) #linear model
...
inquiries <- subset(loan , 
                    total.inquiries > quantile(loan$total.inquiries , 
                                               0.05 , na.rm = TRUE) &
                      total.inquiries <quantile(loan$total.inquiries,
                                                0.95,na.rm = TRUE))
#Quantile >5th <95th
...
cor.test(inquiries$prosper.rating..numeric. , 
         inquiries$total.inquiries) #correlation
...
cor.test(inquiries$total.inquiries , inquiries$borrower.rate) #Correlation

lmfit <- lm(borrower.rate ~ total.inquiries , inquiries) #Linear model
...
ggplot(data = loan , aes(x = current.credit.lines , y = monthly.loan.payment)) + 
  geom_jitter(alpha = 0.02 , position = position_jitter(h = 0)) +
  geom_smooth() +
...
loan.temp <- subset(loan , current.credit.lines > 
                      quantile(loan$current.credit.lines , 0.05 ,
                               na.rm = TRUE) &
                     current.credit.lines < quantile(loan$current.credit.lines ,
                                                     0.95 , na.rm = TRUE))

#Creating temporary loan.temp df , for quantiled data
...
debt.temp$home <- ifelse(debt$is.borrower.homeowner == 'True' ,  
                         as.numeric(1) , as.numeric(0))
#Assigning values 0 if not homeowner , 1 if homeowner for correlation ( Cannot use categorical variables)

cor.test(debt.temp$stated.monthly.income , 
         debt.temp$home) #Correlation

lmfit <- lm(home ~ stated.monthly.income , 
            debt.temp) #linear model
...
#Grouping by public record bin
public.record.group.total <- public.record %>%
                              group_by(public.record.bin) %>%
                             summarise(median = median(borrower.rate))

#Return the bin number for which total is x
value <- function(x)
{
  y = NA
...
#For bin x , I'm using bin number , which is the index of the dataframe to correlate.
...
#For total x = 4000 , I would return 2 , since they are in increasing order anyway and would provide a proper correlation

public.record.group.total$new_n <-
  lapply(public.record.group.total$public.record.bin , value)

public.record.group.total$new_n <- 
  as.numeric(public.record.group.total$new_n)

public.record.group.total <- subset(public.record.group.total ,
                                    !is.na(public.record.bin))

cor.test(public.record.group.total$new_n ,
         public.record.group.total$median)

lmfit <- lm(median ~ public.record.group.total$new_n ,
            public.record.group.total)
...
I noticed that for people with lower credit score , the borrower rate is higher than people with better credit scores for the same amount borrowed.
...
ggplot(data = income.groups , aes(x = stated.monthly.income,
  ...
  ylab('Monthly loan Payment(in USD)') + 
...
ggplot(data = monthly.rate , aes(x = monthly.loan.payment , 
  ...
  facet_wrap(~status , scales = 'free') +
  scale_color_gradient(low = 'black' , high = 'green') +
  ylab('Borrower A.P.R(%)') + xlab('Monthly loan payment(in USD)') + 
...
ggplot(data = amount.rate , aes(x = loan.original.amount , y = borrower.rate)) + 
  geom_jitter(aes(color = credit.score.exp) ,
              position = position_jitter(h = 0) , alpha = I(0.4)) + 
  facet_wrap(~year , scales = 'free_x') +
  scale_color_gradient(low = 'deeppink4', high = 'darkturquoise') + 
  xlab('Loan original amount(in USD)') + ylab('Borrower rate(in %)') + 
ggtitle('Borrower rate vs Loan original amount by Average credit score')
